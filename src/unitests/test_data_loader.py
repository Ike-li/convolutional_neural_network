...
def test_get_mnist_loaders():
    ...
    train_loader, val_loader, test_loader = get_mnist_loaders(
        batch_size=64, test_batch_size=100, val_ratio=0.1, augment=False
    )
...
def test_get_data_loaders():
    ...
    loaders = get_data_loaders(
        dataset_name="mnist", batch_size=64, test_batch_size=100, val_ratio=0.1
    )
    assert len(loaders) == 3

    # 测试Fashion-MNIST
    loaders = get_data_loaders(
        dataset_name="fashion_mnist", batch_size=64, test_batch_size=100, val_ratio=0.1
    )
    assert len(loaders) == 3
    ...
    with pytest.raises(ValueError):
        get_data_loaders(
            dataset_name="unknown_dataset", batch_size=64, test_batch_size=100
        )
